...
export fn savePasteToLastViewed(limit: u32, packed_string: [*]u8, packed_len: usize, paste_json: [*]u8, paste_len: usize) void {
    ...
    var pasteList = std.ArrayList(zpacked.PackedPaste).init(allocator);
    defer pasteList.deinit();

    for (existing.pastes) |p| {
        if (pasteList.items.len + 1 == limit) {
            break;
        }
        pasteList.append(p) catch @panic("failed to append paste");
    }

    pasteList.insert(0, newPaste) catch @panic("failed to append paste");
