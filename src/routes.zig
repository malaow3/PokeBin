...
const StreamContext = struct {
    ...
    fn handle(self: StreamContext, stream: std.net.Stream) void {
        ...
        generateScreenshot(allocator, self.id) catch {
...
fn generateScreenshot(allocator: std.mem.Allocator, id: []const u8) !void {
    zlog.pushScope("screenshot") catch @panic("Failed to push scope");
    defer _ = zlog.popScope() catch @panic("Failed to pop scope");
